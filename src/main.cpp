...
const string strMessageMagic = "RadioShares Signed Message:\n";
...
static const int64 nTargetTimespan = 3 * 30 * 60; // check every 3 blocks
static const int64 nTargetSpacing = 30 * 60; // 1/2 hour
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RadioSharesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RadioSharesMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("RadioSharesMiner started\n");
    ...
    try { for(;;) {
        ...
        printf("Running RadioSharesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RadioSharesMiner terminated\n");
        throw;
    }
}
